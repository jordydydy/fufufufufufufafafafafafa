...
class ChatbotClient:
    ...
    async def ask(
        self, query: str, conversation_id: str, platform: str, user_id: str
    ) -> bool:
        start_timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S.%f")[
            :-3
        ]
        ...
        try:
            await self._fire_request(url, payload, headers)
            return True
        except Exception as e:
