...
def _extract_graph_body(msg):
    ...
    return (
        sanitize_email_body(None, body_content)
        if body_type.lower() == "html"
        else sanitize_email_body(body_content, None)
    )
...
def _process_gmail_message(mail, msg_id):
    try:
        ...
        if any(
            skip in sender_lower
            for skip in ["mailer-daemon", "noreply", "no-reply", "postmaster"]
        ):
...
def process_single_email(sender_email, body, metadata: dict):
    ...
    msg = IncomingMessage(
        platform_unique_id=sender_email, query=body, platform="email", metadata=metadata
    )
